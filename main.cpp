...
using Vertex = list<array<fat4, 3>, array<fat4, 2>>;
...
bool InitD3D()
{
	...
	if (FAILED(hr)) return Running = false;
	...
	hr = yw::main::system::create_committed_resource(
		CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE,
		CD3DX12_RESOURCE_DESC::Buffer(vBufferSize), D3D12_RESOURCE_STATE_COPY_DEST, vertexBuffer);
	if (FAILED(hr)) return Running = false;
	// ok(source{}.to_string());
	...
	hr = yw::main::system::create_committed_resource(
		...
		CD3DX12_RESOURCE_DESC::Buffer(vBufferSize), D3D12_RESOURCE_STATE_GENERIC_READ, vBufferUploadHeap);
	if (FAILED(hr)) return Running = false;
	...
	yw::main::system::resource_barrier(CD3DX12_RESOURCE_BARRIER::Transition(vertexBuffer, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER));
	...
	hr = yw::main::system::create_committed_resource(
		CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE,
		CD3DX12_RESOURCE_DESC::Buffer(iBufferSize), D3D12_RESOURCE_STATE_COPY_DEST, indexBuffer);
	if (FAILED(hr)) return Running = false;
	// ok(source{}.to_string());
	...
	hr = yw::main::system::create_committed_resource(
		CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), D3D12_HEAP_FLAG_NONE,
		CD3DX12_RESOURCE_DESC::Buffer(vBufferSize), D3D12_RESOURCE_STATE_GENERIC_READ, iBufferUploadHeap);
	if (FAILED(hr)) return Running = false;
	...
	yw::main::system::resource_barrier(CD3DX12_RESOURCE_BARRIER::Transition(indexBuffer, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER));
	...
	hr = yw::main::system::create_committed_resource(
		CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE,
		CD3DX12_RESOURCE_DESC::Tex2D(DXGI_FORMAT_D32_FLOAT, Width, Height, 1, 0, 1, 0, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL),
		D3D12_RESOURCE_STATE_DEPTH_WRITE, depthOptimizedClearValue, depthStencilBuffer);
	if (FAILED(hr)) return Running = false;
	...
	for (int i = 0; i < frameBufferCount; ++i)
	{
		...
		hr = yw::main::system::create_committed_resource(
			CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), D3D12_HEAP_FLAG_NONE,
			CD3DX12_RESOURCE_DESC::Buffer(D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT), D3D12_RESOURCE_STATE_GENERIC_READ, constantBufferUploadHeaps[i]);
		if (FAILED(hr)) return Running = false;
	...
	hr = yw::main::system::create_committed_resource(
		CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE,
		textureDesc, D3D12_RESOURCE_STATE_COPY_DEST, textureBuffer);
	if (FAILED(hr)) return Running = false;
	...
	hr = yw::main::system::create_committed_resource(
		CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), D3D12_HEAP_FLAG_NONE,
		CD3DX12_RESOURCE_DESC::Buffer(textureUploadBufferSize), D3D12_RESOURCE_STATE_GENERIC_READ, textureBufferUploadHeap);
	if (FAILED(hr)) return Running = false;
	...
	yw::main::system::resource_barrier(CD3DX12_RESOURCE_BARRIER::Transition(textureBuffer, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE));
	...
	// make sure we have data
	if (int8(fontImageData) <= 0) return Running = false;
	...
	hr = yw::main::system::create_committed_resource(
		CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE,
		fontTextureDesc, D3D12_RESOURCE_STATE_COPY_DEST, arialFont.textureBuffer);
	if (FAILED(hr)) return Running = false;
	...
	hr = yw::main::system::create_committed_resource(
		CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), D3D12_HEAP_FLAG_NONE,
		CD3DX12_RESOURCE_DESC::Buffer(fontTextureUploadBufferSize), D3D12_RESOURCE_STATE_GENERIC_READ, fontTextureBufferUploadHeap);
	if (FAILED(hr)) return Running = false;
	...
	yw::main::system::resource_barrier(CD3DX12_RESOURCE_BARRIER::Transition(arialFont.textureBuffer, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE));
	...
	for (int i = 0; i < frameBufferCount; ++i)
	{
		...
		hr = yw::main::system::create_committed_resource(
			CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), D3D12_HEAP_FLAG_NONE,
			CD3DX12_RESOURCE_DESC::Buffer(maxNumTextCharacters * sizeof(TextVertex)), D3D12_RESOURCE_STATE_GENERIC_READ, textVertexBuffer[i]);
		if (FAILED(hr)) return Running = false;
		// ok(source{}.to_string());
...
void Update(double delta)
{
	...
	XMMATRIX rotXMat = XMMatrixRotationX(float(0.001f * delta));
	XMMATRIX rotYMat = XMMatrixRotationY(float(0.002f * delta));
	XMMATRIX rotZMat = XMMatrixRotationZ(float(0.003f * delta));
	...
	rotXMat = XMMatrixRotationX(float(0.003f * delta));
	rotYMat = XMMatrixRotationY(float(0.002f * delta));
	rotZMat = XMMatrixRotationZ(float(0.001f * delta));
...
void UpdatePipeline()
{
	...
	yw::main::system::resource_barrier(CD3DX12_RESOURCE_BARRIER::Transition(renderTargets[frameIndex], D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET));
	...
	yw::main::system::resource_barrier(CD3DX12_RESOURCE_BARRIER::Transition(renderTargets[frameIndex], D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT));
