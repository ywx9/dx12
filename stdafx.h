...
#define ywlib_disable_base_system
#define ywlib_disable_main_window
#define ywlib_disable_directx
#include "ywlib"
using namespace yw;
...
namespace yw::main::system {

template<is_pointer T> inline HRESULT create_committed_resource(
	D3D12_HEAP_PROPERTIES Properties, D3D12_HEAP_FLAGS Flags, D3D12_RESOURCE_DESC Desc,
	D3D12_RESOURCE_STATES State, D3D12_CLEAR_VALUE ClearValue, T& Resource) {
	return device->CreateCommittedResource(&Properties, Flags, &Desc, State, &ClearValue, IID_PPV_ARGS(&Resource));
}

template<is_pointer T> inline HRESULT create_committed_resource(
	D3D12_HEAP_PROPERTIES Properties, D3D12_HEAP_FLAGS Flags, D3D12_RESOURCE_DESC Desc,
	D3D12_RESOURCE_STATES State, T& Resource) {
	return device->CreateCommittedResource(&Properties, Flags, &Desc, State, nullptr, IID_PPV_ARGS(&Resource));
}

inline void resource_barrier(D3D12_RESOURCE_BARRIER Barrier) {
